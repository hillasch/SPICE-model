...
def load_pair_from_csv(csv_path: Path, row_index: int = 9):
    ...
    row = rows[row_index]
    src_path = Path(row.get("src_local_path") or "").expanduser()
    comp_path = Path(row.get("comp_local_path") or "").expanduser()
    prompt = row.get("llm_edit", "").strip()

    if not src_path.exists():
        raise FileNotFoundError(f"Missing source image at {src_path}")
    if not comp_path.exists():
        raise FileNotFoundError(f"Missing comparison/target image at {comp_path}")
